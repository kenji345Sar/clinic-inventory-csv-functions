...
- **文字コードの食い違いはファイル単位で止める**。卸ごとに文字コードは固定で宣言しているが、
  申告と違うものが届くことがある。デコーダは変換できない文字をエラーにせず置換文字にして
  進めてしまうため、そのまま通すと**商品名が化けたまま取り込みが成功する**。
  変換後に置換文字が出た場合と、Shift_JIS指定なのに中身がUTF-8として読める場合を検出し、
  要確認にして止める（[csv_util.go](../internal/infrastructure/distributorcsvingestion/csv_util.go)の`decode`）。
